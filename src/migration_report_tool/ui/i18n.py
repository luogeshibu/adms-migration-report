...
ZH_CN: dict[str, str] = {
    ...
    "Every mapped App field can be shown or hidden here. SYSTEM calculation fields are always visible because they feed matching, Analysis or validation. Optional built-in fields and USER App columns are presentation choices only. New App fields appear automatically and can be hidden later. No source mapping, SQLite review data or exported report history is deleted by changing this view.": "这里可以选择显示或隐藏每个已映射的 App 字段。参与匹配、分析或校验的 SYSTEM 计算字段必须始终显示，不能隐藏。可选内置字段和 USER App 字段仅影响界面展示。以后新增的 App 字段会自动出现，也可以再手动隐藏。修改这里的显示不会删除源字段映射、SQLite 审核数据或历史导出记录。",
    ...
    "Setup complete. Source Workspace and Project Data Storage are configured and will be reused automatically on future launches.": "设置已完成。数据源工作区和项目数据存储位置已配置，后续启动时会自动继续使用。",
    "Source Workspace / Site Repository is read-only input. Project Data Storage is writable persistent application state stored separately from the release and source folders and survives upgrades. Each site keeps project.db, Review/Closed/Needs Action, Comments, Resolution, Change Audit, named revisions, issue/action records, generated sign-off PDFs, signed PDF copies, imported source copies and snapshots under Project Data. Future SQLite schema upgrades are forward-only and create a backup before changing project.db. Automatic validation and review behavior remain unchanged.": "数据源工作区 / 站点数据仓库为只读输入。项目数据存储区用于持久保存可写的应用状态，与程序发布目录和源文件目录分离，升级程序时不会丢失。每个站点都会在项目数据中保存 project.db、审核状态（Review/Closed/Needs Action）、备注、处理决议、变更审计、命名审核版本、问题/处理记录、生成的签字 PDF、已签字 PDF 副本、导入的数据源副本和快照。未来 SQLite 数据库结构升级仅向前兼容，并会在修改 project.db 前自动备份；自动校验和审核业务逻辑保持不变。",
...
ZH_CN.update({
    "Fields Used in This Module (App ← Source)": "本模块使用字段（App ← 源字段）",
    "Each row is one real source table. CSV and Excel (.xlsx/.xlsm) are supported for every site source role. Physical filenames are not business constraints: manual assignment plus field mapping is authoritative, and explicit Refresh can discover arbitrary filenames by schema fields. AUTO still uses published V versions / detection rules as convenient discovery hints. Click Source File to return to AUTO, pin a version, or browse any supported table. For Excel, Sheet defaults to AUTO (first usable sheet in workbook order) and can be pinned per site without changing the global field mapping. File Path shows the reviewer-facing location. Fields Used shows exactly which App fields this module reads from that file. Missing source fields stay blank. Changing one Source File re-reads only that table. Refresh Sources re-scans versions and re-reads changed files; Run Validation force re-reads all active files. While the App is open, live file metadata is watched and changed inputs are refreshed in the background.": "每一行对应一个真实数据源表。所有站点数据源都支持 CSV 和 Excel（.xlsx/.xlsm）。物理文件名不是业务限制：手动指定数据源并完成字段映射后，该映射就是实际使用依据；主动刷新也可以根据字段结构识别任意文件名。AUTO 仍会优先参考已发布的 V 版本和数据源识别规则；点击‘数据源文件’可恢复 AUTO、固定某个版本或浏览任意受支持的表格文件。Excel 的工作表默认使用 AUTO（按工作簿顺序选择第一个可用工作表），也可以按站点固定工作表，而且不会改变全局字段映射。‘文件路径’显示审核人员实际使用的文件位置；‘本模块使用字段’显示 App 字段与物理源字段的实际对应关系。缺失的源字段保持为空。更换一个数据源文件只重新读取该表；‘刷新数据源’会重新扫描版本并读取发生变化的文件；‘运行校验’会强制重新读取所有当前数据源。应用运行期间还会监视文件元数据，并在输入发生变化时自动后台刷新。",
...
ZH_CN.update({
    ...
    "The export contains exactly five sheets. RMU Data Review and Signal Mapping Review are generated from the App review models; STANDARD is copied from the currently active application reference; Import Sources and Change Audit Log provide traceability.": "导出的工作簿固定包含五个工作表。设备数据审核和信号映射审核由 App 审核模型生成；STANDARD 从当前启用的应用级参考文件复制；Import Sources 和 Change Audit Log 用于追溯。",
    ...
    "Choose one resolution for each FALSE Analysis field. Customer Comments are optional and independent from the Resolution choice; changing a comment never changes Analysis or the selected Resolution. Both the latest comment and every historical comment change are retained. Where ADMS DB provides the reference value, choosing the same normalized value closes the equipment review; choosing a different value sets Review to Needs Action. Unresolved remains Unreviewed.": "请为每个 Analysis=FALSE 的字段选择一个处理决议。客户备注为可选项，并与处理决议相互独立；修改备注不会改变 Analysis 或已选择的处理决议。最新备注和全部历史备注都会保留。当 ADMS DB 提供参考值时，选择与其标准化后相同的值会关闭该设备审核；选择不同值会将审核状态设为“需处理”；未处理则保持“未审核”。",
